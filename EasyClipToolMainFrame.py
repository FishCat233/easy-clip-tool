...
class ConfigTool:
    ...
    @staticmethod
    def getTimeStrByStr(val: str) -> str:
        """
        读取各种风格的字符串，转化为命令行中使用的字符串
        :param self:
        :param val: 各种风格的字符串
        :return:
        """
    ...
    @staticmethod
    def timeStrIsValid(val: str) -> bool:
        """
        判断时间字符串是否合法
        :param val:
        :return: True/False
        ...
        try:
            # 尝试将时间字符串转换为datetime对象，格式为'M:S'
            time_obj = datetime.strptime(val, '%M:%S')
            return True
        except ValueError:
            # 如果转换失败，则说明时间字符串不合法
            pass
        try:
            # 尝试将时间字符串转换为datetime对象，格式为'H:M:S'
...
class Config:
    ...
    def __init__(self):
        ...
        self.__output_mbps = 6
        self.__output_path = ''

    def getOutputFilename(self) -> str:
        if self.__output_filename == '':
            self.__output_filename = str(time.strftime('No Title %Y.%m.%d - %H.%M.%S.output.mp4'))
        return self.__output_filename

    def setOutputFilename(self, val: str) -> None:
        self.__output_filename = val

    def getOutputMbps(self) -> int:
        return self.__output_mbps

    def setOutputMbps(self, val: int) -> None:
        if val <= 0:
            val = 1
        self.__output_mbps = val
...
class EasyClipToolMainFrame(EasyClipTool.MainFrame):
    ...
    def AddFileBtnOnClick(self, event):
        ...
        if file_dlg.ShowModal() == wx.ID_OK:
            ...
            logging.debug("{}, {}".format(file_dlg.GetFilename(), file_dlg.GetPath()))
    ...
    def ExportBtnOnClick(self, event):
        ...
        logging.info(f"Running with command: {console_command}")
